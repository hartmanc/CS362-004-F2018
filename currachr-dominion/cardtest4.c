...
int main() {
    ...
    */
    ...
    cardEffect(adventurer, 0, 0, 0, state, 0, &maxBonus); /* handPos = 0 will break state */
    ...
    if (origHandCount == state->handCount[whoseTurn(state)])
        printf("buyCard: PASS - Expected number of extra cards\n");
    else
        printf("buyCard: FAIL - Unexpected number of extra cards\n");

    /* Did the player get two more actions? */
    if (origActions + 2 == state->numActions)
        printf("buyCard: PASS - Expected number of actions\n");
    else
        printf("buyCard: FAIL - Unexpected number of actions\n");
